...
if (!FileAccess.file_exists("res://node_modules/xlsx/xlsx.js")) {
  console.info(
    "please run 'npm install' in the directory './' at first if 'xlsx' module can not be resolved",
  );
}
...
export function exec() {
  ...
  let wb = xlsx.read(FileAccess.get_file_as_bytes(filename).to_array_buffer(), {
    type: "buffer",
  });
