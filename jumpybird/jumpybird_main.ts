...
export default class JumpyBirdMain extends Node {
  ...
  _ready() {
    ...
    this._scene = <Sprite2D>this.get_node("scene");
    this._bird = <Sprite2D>this._scene.get_node("bird");
    const area = <Area2D>this._bird.get_node("Area2D");
    area.area_shape_entered.connect(
      Callable.create(this, this.on_area_shape_entered),
      0,
    );
  ...
  private add_pipe() {
    ...
    const up_pipe = <JumpyBirdPipe>(
      this.instantiate_asset("res://jumpybird/pipe.tscn")
    );
    const down_pipe = <JumpyBirdPipe>(
      this.instantiate_asset("res://jumpybird/pipe.tscn")
    );
