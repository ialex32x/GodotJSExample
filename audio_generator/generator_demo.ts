...
class Visualizer {
  ...
  private get_level(index: number, rf: number) {
    ...
    for (let i = 0; i < rf; ++i) {
      ...
      value += Math.sqrt(
        this.real[e] * this.real[e] + this.imag[e] * this.imag[e],
      );
    }
...
export default class AudioGeneratorDemo extends Node {
  ...
  _ready(): void {
    ...
    this.player.play();
    this.playback = <AudioStreamGeneratorPlayback>(
      this.player.get_stream_playback()
    );
    ...
    if (typeof info.fully_loaded === "undefined") {
      ...
      for (let i = 0; ; ++i) {
        ...
        if (!(i % galop)) {
          ...
          label.text = this.loading
            ? `${info.name} (loading... ${i / galop})`
            : info.name;
  ...
  _process(delta: number): void {
    ...
    if (
      this.loading &&
      (typeof info.cache === "undefined" || info.cache[0].length < 100_000)
    ) {
      return;
    }
