...
const TAB = "    ";
const Util = {
  to_native: function (type: string) {
    return `StaticBindingUtil<${type}>::get`;
  },
  to_script: function (type: string) {
    return `StaticBindingUtil<${type}>::set`;
  },
  to_this: function (type: string) {
    return `PrimitiveInstanceUtil<${type}>::get`;
  },
};
const HEADERS = [
  "jsb_class_info.h",
...
class V8LazyDataPropertyWriter extends IndentWriter {
  constructor(parent: IWriter, name: string) {
    ...
    this.parent.line(
      `static void ${name}(v8::Local<v8::Name> name, const v8::PropertyCallbackInfo<v8::Value>& info)`,
    );
    this.parent.line(`{`);
    this.line("v8::Isolate* isolate = info.GetIsolate();");
    this.line("v8::Local<v8::Context> context = isolate->GetCurrentContext();");
...
class PrimitiveClassGen {
  ...
  private emit_constructor_body(
    body: IWriter,
    ctor: IConstructor,
    index: number,
  ) {
    const inner = new IfWriter(
      body,
      `v8_argc == ${ctor.argc}`,
      index == 0 ? "if" : "else if",
    );
    if (ctor.overloads.length == 1) {
      ...
      ctor.overloads[0].arguments.forEach((arg, index) => {
        ...
        IF(
          inner,
          `!${Util.to_native(arg_type)}(isolate, context, info[${index}], ${arg_name})`,
        ).THEN(`jsb_throw(isolate, "bad param at ${index}");`, `return;`);
        args.push(arg_name);
      });
      inner.line(`Variant* instance = Environment::alloc_variant();`);
      inner.line(
        `*instance = ${to_cpp_type(this.type.type)}(${args.join(", ")});`,
      );
      inner.line(
  ...
  emit_combined_constructor(struct: IWriter) {
    ...
    this.type.constructors?.sort(
      (a, b) => a.arguments.length - b.arguments.length,
    );
    this.type.constructors?.forEach((value) => {
      if (
        combined.length == 0 ||
        combined[combined.length - 1].argc != value.arguments.length
      ) {
    ...
    combined.forEach((value, index) =>
      this.emit_constructor_body(func, value, index),
    );
  ...
  emit_method(
    struct: IWriter,
    method: editor.MethodBind,
    generated_methods: Array<GeneratedMethod>,
  ) {
    let args: Array<string> = [];
    ...
    if (method.is_static) {
      ...
      IF(
        func,
        `!${Util.to_this(self_type)}(isolate, context, info.This(), thiz)`,
      ).THEN(`jsb_throw(isolate, "failed to get this");`, `return;`);
    ...
    if (
      typeof method.return_ != "undefined" &&
      method.return_.type != Variant.Type.TYPE_NIL
    ) {
      const return_type = to_cpp_type(method.return_.type);
      func.line(`v8::Local<v8::Value> rval;`);
      IF(
        func,
        `${Util.to_script(return_type)}(isolate, context, ${op_call}${method.name}(${args.join(", ")}), rval)`,
      ).THEN(
  ...
  emit_property(
    struct: IWriter,
    info: editor.PrimitiveGetSetInfo,
    generated_methods: Array<GeneratedProperty>,
  ) {
    ...
    const self_type = to_cpp_type(this.type.type);
    {
      ...
      IF(
        getter,
        `${Util.to_script(arg_type)}(isolate, context, ((const ${self_type}*) thiz)->${this.get_underlying_getter(info.name)}(), rval)`,
      ).THEN(
        `jsb_throw(isolate, "failed to translate return value");`,
        `return;`,
      );
    ...
    if (typeof generated.setter_name !== "undefined") {
      ...
      IF(
        setter,
        `!${Util.to_native(arg_type)}(isolate, context, info[0], rval)`,
      ).THEN(
        `jsb_throw(isolate, "failed to translate return value");`,
        `return;`,
      );
  ...
  emit_combined_operator(
    struct: IWriter,
    generated_operators: Array<GeneratedOperator>,
  ) {
    ...
    this.type.operators?.forEach((value) => {
      ...
      if (typeof operator === "undefined") {
        operator = {
          name: value.name,
          is_unary: value.right_type == Variant.Type.TYPE_NIL,
          overloads: [],
        };
        map.set(value.name, operator);
      }
      console.assert(
        operator.is_unary == (value.right_type == Variant.Type.TYPE_NIL),
      );
    ...
    for (let kv of map) {
      if (kv[1].is_unary) {
        console.assert(
          kv[1].overloads.length == 1,
          "overloading on unary operator is invalid",
        );
        this.emit_unary_operator_body(
          struct,
          kv[1].overloads[0],
          generated_operators,
        );
  ...
  emit_binary_operator_body(
    ...
    if (info.overloads.length == 1) {
      ...
      IF(
        func,
        `!${Util.to_native(left_type)}(isolate, context, info[0], ${left_name}) || !${Util.to_native(right_type)}(isolate, context, info[1], ${right_name})`,
      ).THEN(`jsb_throw(isolate, "bad param");`, `return;`);
      func.line(
        `${return_type} return_value = ${left_name} ${cpp_op} ${right_name};`,
      );
      func.line(`v8::Local<v8::Value> rval;`);
      IF(
        func,
        `${Util.to_script(return_type)}(isolate, context, return_value, rval)`,
      ).THEN(
        `jsb_throw(isolate, "failed to translate return value");`,
        `return;`,
      );
  ...
  emit_unary_operator_body(
    struct: IWriter,
    info: editor.OperatorInfo,
    generated_operators: Array<GeneratedOperator>,
  ) {
    const generated: GeneratedOperator = {
      method_name: `op_${info.name}`,
    ...
    IF(
      func,
      `!${Util.to_native(arg_type)}(isolate, context, info[0], ${arg_name})`,
    ).THEN(`jsb_throw(isolate, "bad param");`, `return;`);
  ...
  emit_lazy_constant(writer: IWriter) {
    ...
    func.line(
      `const Variant constant_value = Variant::get_constant_value((Variant::Type) ${this.type.type}, constant, &r_valid);`,
    );
    func.line(`jsb_check(r_valid);`);
    func.line(`v8::Local<v8::Value> rval;`);
    IF(
      func,
      `!Realm::gd_var_to_js(isolate, context, constant_value, rval)`,
    ).THEN(`jsb_throw(isolate, "bad translate");`, `return;`);
    func.line(`info.GetReturnValue().Set(rval);`);
    func.finish();
  }
  ...
  emit(): ITypeRegister {
    const self_type = to_cpp_type(this.type.type);
    let struct = new StructWriter(
      this.codegen,
      `${this.type.name}StaticBinding`,
    );
    ...
    this.type.methods.forEach((value) =>
      this.emit_method(struct, value, generated_methods),
    );
    this.type.properties?.forEach((value) =>
      this.emit_property(struct, value, generated_properties),
    );
...

    {
      ...
      generated_methods.forEach((value) => {
        if (value.method_info.is_static) {
          bind.line(
            `function_template->Set(V8Helper::to_string(p_env.isolate, "${value.method_info.name}"), v8::FunctionTemplate::New(p_env.isolate, &${value.method_name}));`,
          );
        } else {
          bind.line(
            `prototype_template->Set(V8Helper::to_string(p_env.isolate, "${value.method_info.name}"), v8::FunctionTemplate::New(p_env.isolate, &${value.method_name}));`,
          );
        }
      });
      ...
      for (let enum_info of this.type.enums ?? []) {
        ...
        block.line(
          `function_template->Set(V8Helper::to_string(p_env.isolate, "${enum_info.name}"), enum_obj);`,
        );
        for (let enum_item of Object.keys(enum_info.literals)) {
          let enum_value = this.type.constants!.find(
            (v) => v.name == enum_item,
          )!.value;
          block.line(
            `enum_obj->Set(V8Helper::to_string(p_env.isolate, "${enum_item}"), v8::Int32::New(p_env.isolate, ${enum_value}));`,
          );
        ...
        for (let const_info of this.type.constants) {
          ...
          switch (const_info.type) {
            case Variant.Type.TYPE_FLOAT:
              block.line(
                `[NOT_IMPLEMENTED] SET_CONST_NUMBER(${const_info.name}, ${const_info.value});`,
              );
              break;
            case Variant.Type.TYPE_INT:
              block.line(
                `[NOT_IMPLEMENTED] SET_CONST_INT(${const_info.name}, ${const_info.value});`,
              );
              break;
            default:
              block.line(
                `function_template->SetLazyDataProperty(V8Helper::to_string(p_env.isolate, "${const_info.name}"), &ConstantAccess<${self_type}>::_get);`,
              );
              break;
          }
        }
      ...
      generated_operators.forEach((value) => {
        bind.line(
          `function_template->Set(V8Helper::to_string(p_env.isolate, "${value.op_name}"), v8::FunctionTemplate::New(p_env.isolate, &${value.method_name}));`,
        );
      });
      ...
      generated_properties.forEach((value) => {
        bind.line(
          `prototype_template->SetAccessorProperty(V8Helper::to_string(p_env.isolate, "${value.name}"),`,
        );

        let indent = new IndentWriter(bind);
        indent.line(
          `v8::FunctionTemplate::New(p_env.isolate, &${value.getter_name}),`,
        );
        if (typeof value.setter_name !== "undefined") {
          indent.line(
            `v8::FunctionTemplate::New(p_env.isolate, &${value.setter_name})`,
          );
        }
        indent.finish();
        bind.line(");");
      ...
      if (typeof this.type.element_type != "undefined") {
        ...
        bind.line(
          `prototype_template->Set(V8Helper::to_string(p_env.isolate, "set_indexed"), v8::FunctionTemplate::New(p_env.isolate, &IndexAccess<${self_type}>::_set_indexed));`,
        );
        bind.line(
          `prototype_template->Set(V8Helper::to_string(p_env.isolate, "get_indexed"), v8::FunctionTemplate::New(p_env.isolate, &IndexAccess<${self_type}>::_get_indexed));`,
        );
      }
    ...
    return {
      type: this.type.type,
      method_name: `${this.type.name}StaticBinding::static_bind`,
    };
  }
}
...
class Generator {
  ...
  emit() {
    ...
    {
      let codegen = new ConditionalCompiledWriter(
        file_writer,
        "JSB_WITH_STATIC_BINDINGS",
      );
      ...
      let binding = new CFunctionWriter(
        ns,
        "void register_primitive_bindings_static(class Realm* p_realm)",
      );
      for (let register of generated_register) {
        binding.line(
          `p_realm->add_class_register((Variant::Type) ${register.type}, &${register.method_name});`,
        );
      }
      binding.finish();
      ns.finish();
