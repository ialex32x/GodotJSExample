import { Area2D, Callable, Node2D } from "godot";

export default class Wolf extends Node2D {
  _ready() {
    console.log("wolf.ready");
    try {
      let balloon = <Area2D>this.get_node("Area2D");
      console.log(typeof balloon.area_shape_entered);
      balloon.area_shape_entered.connect(
        Callable.create(this, this.on_balloon_collision),
      );
